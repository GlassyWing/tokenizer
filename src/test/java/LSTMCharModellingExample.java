...
/**
 * Create by manlier 2018/7/31 15:46
 */
public class LSTMCharModellingExample {
     ...
     * @param initialization String, may be null, If null, select random character as initialization for all samples
     ...
     * @param numSamples Number of sample
     */
    private static String[] sampleCharactersFromNetwork(String initialization, MultiLayerNetwork net,
        ...
        for (int i = 0; i < init.length; i++) {
            ...
            for (int j = 0; j < numSamples; j++) {
                initializationInput.putScalar(new int[]{j, idx, i}, 1.0);
            }
        }
    ...
    public static void run() throws Exception {
        int lstmLayerSize = 200;        // Number of unit in each LSTM layer
        int miniBatchSize = 32;         // Size of mini batch to use when traning
        int exampleLength = 1000;       // Length of each training example sequence to use. This could certainly be increased
        int tbpttLength = 50;           // Length for truncated backpropagation through time. i.e., do parameter updates ever 50 characters
        int numEppochs = 1;             // Total number of training epochs
        int generateSampleEveryNMinibatches = 10; //
        ...
        MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
                ...
                .layer(2, new RnnOutputLayer.Builder(LossFunctions.LossFunction.MCXENT)
                        .activation(Activation.SOFTMAX).nIn(lstmLayerSize).nOut(nOut)
                        .build())
        ...
        for (int i = 0; i < numEppochs; i++) {
            while (iter.hasNext()) {
                DataSet ds = iter.next();
                net.fit(ds);
                if (++miniBatchNumber % generateSampleEveryNMinibatches == 0) {
                    System.out.println("------------------------");
                    System.out.println(String.format("Completed %d minibatches of size %dx%d + characters",
                            miniBatchNumber, miniBatchSize, exampleLength));
                    System.out.println(String.format("Sampling characters from network given initialization \"%s",
                            generationInitialization == null ? "" : generationInitialization) + "\"");
                    String[] samples = sampleCharactersFromNetwork(generationInitialization, net,
                            iter, rng, nCharactersToSample, nSamplesToGenerate);
