...
/**
 * Create by manlier 2018/8/1 9:14
 */
public class Ex1BasicSameDiffLayerExample {

    public static void main(String[] args) throws Exception {

        int networkNumInputs = 28 * 28;          // For MNIST - 28 x 28 pixels
        ...
        MultiLayerConfiguration conf = new
                ...
                .seed(12345)
                .list()
                .layer(new MinimalSameDiffDense(networkNumInputs, layerSize, Activation.TANH,
                        WeightInit.XAVIER))
                .layer(new MinimalSameDiffDense(layerSize, layerSize, Activation.TANH,
                        WeightInit.XAVIER))
                .layer(new OutputLayer.Builder().nIn(layerSize).nOut(networkNumOutputs)
    ...
    public static void validateLayer() throws Exception {
        /*
        when implementing a custom layer, it is a good idea to validate the iplementation.
        Specifically:
        1. Checking that model serialization(i.e., saving adn loading) works
        ...
        double gradient_check_epsilon = 1e-6;
        // Epsilon value used for gradient checks
        double max_relative_error = 1e-5;
        // Maximum relative error allowable for each parameter
        double min_absolute_error = 1e-8;
        // Minimum absolute error, to avoid failures on 0 vs 1e-30, for example.

        boolean gradOk = GradientCheckUtil.checkGradients(net, gradient_check_epsilon, max_relative_error,
                min_absolute_error, print, return_on_first_failure, testFeatures, testLabels);
        if (!gradOk) {
